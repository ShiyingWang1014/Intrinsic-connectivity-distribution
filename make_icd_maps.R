...
print("This is version 0 of ICD code. Future versions may be created as variants of this code. Main features are:")
print("(1) Subbricks of AFNI time series are not demeaned.")
print("(2) Survival function has form exp(-(1/a)*x^b+c).")
print("(3) Jacobian of survival function is evaluated with numDeriv package.")
print("(4) Initial guesses for parameters are (0.5, 0.5, 0.5) and boundary condition is (0, 0, -Inf)")
...
make_icd = function(fn.4d, fn.gm, fn.prefix, pwd.results){
  # input/output and visualization of medical imaging data
  require(oro.nifti)
...
compute.ICD = function(j, TS.NORM, nbreaks = 201){
  ...
  if(sd(Xj) == 0){
    ...
    Xej = TS.NORM[-j,]
    # Compute correlation between voxel j and other voxels.  # why over n-1
